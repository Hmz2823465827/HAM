...
import com.jxufe.ham.common.entity.PropertyFilter.MatchType;
 ...
 * @author halu
 */
public class HibernateDao<E extends BaseBean, PK extends Serializable> extends SimpleBaseDao<E, PK> {
	
//	private Log log = LoggerFactory.getLogger(H)
	...
	public Page<E> findPage(final Page<E> page, final String hql, final Object... values) {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Page<E> findPage(final Page<E> page, final String hql, final Map<String, ?> values) {
		Assert.notNull(page, "page不能为空");
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Page<E> findPage(final Page<E> page, final Criterion... criterions) {
		Assert.notNull(page, "page不能为空");
	 ...
	 * 设置分页参数到Query对象,辅助函数.
	 */
	protected Query setPageParameterToQuery(final Query q, final Page<E> page) {
	 ...
	 */
	protected Criteria setPageParameterToCriteria(final Criteria c, final Page<E> page) {
	...
	protected long countCriteriaResult(final Criteria c) {
		...
		try {
			...
			Reflections.setFieldValue(impl, "orderEntries", new ArrayList<Object>());
		} catch (Exception e) {
		...
		try {
			...
			logger.error("不可能抛出的异常:{}", e.getCause());
		}
	 ...
	 */
	public List<E> findBy(final String propertyName, final Object value, final MatchType matchType) {
	 ...
	 * 按属性过滤条件列表查找对象列表.
	 */
	public List<E> find(List<PropertyFilter> filters) {
